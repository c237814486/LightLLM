...
class DPDA:
    ...
    def _get_none_jump(self, node_id: int, input_t: T) -> List[DPDAEdge]:
        ...
        self._gen_none_jump_rec(
            node_id,
            node_id,
            graph_node.t_to_item_la[input_t],
            input_t,
            -1,
            pop_list,
            visit_set_state,
    ...
    def dfs_to_find_circle(
        ...
        visit_state_dict: Dict[int, int],
        ans_circles: List[List[int]],
    ):
    ...
    def to_mermaid(self):
        ...
        for edge_maps in self.one_step_node_id_to_dpda_edges.values():
            for (
                input_t_or_nt,
                pop_to_dpda_edge_dict,
            ) in edge_maps.lookah_pop_to_edge.items():
