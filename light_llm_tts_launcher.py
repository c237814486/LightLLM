...
def launch_tts_service(args):
    ...
    # 构建启动命令
    if getattr(sys, "frozen", False):
        from light_tts.server.api_server import parse_args, normal_start

        args.mode = args.tts_mode
...
def main():
    ...
    parser.add_argument("--run_mode", type=str, default="normal", help="LLM运行模式 (normal/prefill/decode等)")
