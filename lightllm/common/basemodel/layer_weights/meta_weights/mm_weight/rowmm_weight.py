...
class UnquantizedROWMMWeight(MMWeightTpl):
    ...
    def _slice_weight(self, weight: torch.Tensor):
        assert (
            weight.shape[0] % self.tp_world_size_ == 0
        ), f"tp slice error {weight.shape[0]} % {self.tp_world_size_}"
        tp_size = weight.shape[0] // self.tp_world_size_
        return weight[tp_size * self.tp_rank_ : tp_size * (self.tp_rank_ + 1)].to(
            self.data_type_
        )

    def _slice_bias(self, bias):
        assert (
            bias.shape[0] % self.tp_world_size_ == 0
        ), f"tp slice error {bias.shape[0]} % {self.tp_world_size_}"
        tp_size = bias.shape[0] // self.tp_world_size_
        return bias[tp_size * self.tp_rank_ : tp_size * (self.tp_rank_ + 1)].to(
            self.data_type_
        )
...
class W8A8B128ROWMMWeight(UnquantizedROWMMWeight):
    def __init__(
        ...
        super().__init__(
            weight_name, data_type, bias_name, quant_method, tp_rank, tp_world_size
        )
...
class UnquantizedMultiROWMMWeight(MultiMMWeightTpl):
    ...
    def __init__(
        ...
        super().__init__(
            weight_names, data_type, bias_names, quant_method, tp_rank, tp_world_size
        )
...
class W8A8B128MultiROWMMWeight(UnquantizedMultiROWMMWeight):
    ...
    def __init__(
        ...
        for weight_name in weight_names:
            weight_scale_name, act_scale_name = generate_scale_name(
                weight_name,
                quant_method.weight_scale_suffix,
                quant_method.act_scale_suffix,
            )
    ...
    def _fuse_weights(self) -> None:
        ...
        if self.weight_scale is None and (None not in self.weight_scales):
            weight_scale = torch.cat(self.weight_scales, dim=0).cuda(
