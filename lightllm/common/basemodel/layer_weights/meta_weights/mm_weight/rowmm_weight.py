...
class W8A8B128ROWMMWeight(UnquantizedROWMMWeight):
    def __init__(
        ...
        self.weight_scale_name, _ = generate_scale_name(weight_name, quant_method.weight_scale_suffix, quant_method.act_scale_suffix)
    ...
    def _slice_weight_scale(self, weight_scale: torch.Tensor):
        assert weight_scale.shape[0] % self.tp_world_size_ == 0, f"tp slice error {weight_scale.shape[0]} % {self.tp_world_size_}"
...
class W8A8B128ROWBMMWeight(UnquantizedROWBMMWeight):
    ...
    def __init__(
        ...
        super().__init__(weight_name, data_type, bias_name, quant_method, tp_rank, tp_world_size)
        self.weight_scale_name, self.act_scale_name = generate_scale_name(weight_name, weight_scale_suffix, act_scale_suffix)
        self.weight_scale: Optional[torch.Tensor] = None
        self.quantized_weight = True
