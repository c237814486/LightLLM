...
class Deepseek2MemoryManager(MemoryManager):
    ...
    def alloc_paged_kv_move_buffer(self, page_num, page_size) -> torch.Tensor:
        self.kv_move_buffer = torch.empty((page_num, page_size, self.layer_num, self.head_num, self.head_dim), dtype=self.dtype, device="cuda")
    ...
    def _get_kv_move_data(self, token_indexes: List[int], layer_index: int):
        ...
        move_buffer = self.kv_move_buffer.view(-1)[0:move_size].view(1, len(token_indexes), self.head_num, self.head_dim)
