...
from lightllm.common.fused_moe.moe_silu_and_mul_mix_quant_ep import (
...
def tma_aligned_quantize(
    ...
    input_scale = torch.empty(
        (m, k // block_size), dtype=torch.float32, device=input_tensor.device
    )
...
def masked_group_gemm(
    ...
    qsilu_out_scale = torch.empty(
        (E, padded_m, N // 2 // block_size),
        device=recv_x[0].device,
        dtype=torch.float32,
    )
    qsilu_out = torch.empty(
        (E, padded_m, N // 2), dtype=w1.dtype, device=recv_x[0].device
    )
    # groupgemm (masked layout)
    gemm_out_b = torch.empty_like(recv_x[0], device=recv_x[0].device, dtype=dtype)

    deep_gemm.m_grouped_gemm_fp8_fp8_bf16_nt_masked(
        recv_x, (w1, w1_scale), gemm_out_a, masked_m, expected_m
    )

    silu_and_mul_masked_post_quant_fwd(
        gemm_out_a, qsilu_out, qsilu_out_scale, block_size, masked_m
    )
    deep_gemm.m_grouped_gemm_fp8_fp8_bf16_nt_masked(
...
def fused_experts_impl(
    ...
    # qaunt hidden_states
    assert (
        use_fp8_w8a8 and use_fp8_all2all
    ), "use_fp8_w8a8 and use_fp8_all2all must be True"
    ...
    if is_prefill:
        ...
        gather_out = torch.empty_like(
            recv_x[0], device=hidden_states.device, dtype=hidden_states.dtype
        )
        if all_tokens > 0:
            input_tensor = [
                torch.empty(
        ...
        expected_m = triton.cdiv(
            hidden_states.shape[0] * buffer.group_size * topk_idx.shape[1], num_experts
        )
