...
class OfflineFP8QuantMemManager(MemoryManager):
    def __init__(
        ...
        if is_export_mode:
            ...
            logger.info(f"kv_quant_calibration_config_path {get_env_start_args().kv_quant_calibration_config_path} is set, " "will load kv quant calibration config")
    ...
    def _load_and_check_config(self):
        if os.path.exists(get_env_start_args().kv_quant_calibration_config_path):
            ...
            if cfg["architectures"] != model_arch:
                raise ValueError(f"architectures {cfg['architectures']} in config " f"not match current model_arch {model_arch}")
            if cfg["num_layers"] != self.layer_num:
                raise ValueError(f"num_layers {cfg['num_layers']} in config " f"not match current layer_num {self.layer_num}")
            if cfg["num_head"] % self.total_head_num != 0 and self.total_head_num % cfg["num_head"] != 0:
                raise ValueError(f"num_head {cfg['num_head']} in config " f"not match current model head num {self.total_head_num}")
            ...
            raise FileNotFoundError(f"kv_quant_calibration_config {get_env_start_args().kv_quant_calibration_config_path} not found")
