...
class OfflineFP8QuantMemManager(MemoryManager):
    def __init__(
        ...
        super().__init__(
            size,
            dtype if is_export_mode else torch.uint8,
            head_num,
            head_dim,
            layer_num,
            always_copy,
        ...
        self.total_head_num = (
            head_num * dist.get_world_size() if dist.is_initialized() else head_num
        )
        self.count = 0
        self.scales = None
        ...
        if is_export_mode:
            ...
            self.scales = torch.tensor(
                self.scales_list, dtype=torch.float32, device="cuda"
            ).view(cfg["scales_shape"])
            ...
            if (
                ...
                v_scales = self.scales[
                    :, start_head + half_head : end_head + half_head
                ].contiguous()
            ...
            logger.warning(
                "scales is None, no kv_quant_calibration_config_path be set, will use 1.0 as scales"
            )
    ...
    def _load_and_check_config(self):
        if os.path.exists(get_env_start_args().kv_quant_calibration_config_path):
            ...
            if cfg["qmin"] != self.qmin:
                raise ValueError(
                    f"qmin {cfg['qmin']} in config not match torch.float8_e4m3fn.min {self.qmin}"
                )
            if cfg["qmax"] != self.qmax:
                raise ValueError(
                    f"qmax {cfg['qmax']} in config not match torch.float8_e4m3fn.max {self.qmax}"
                )
            ...
            if cfg["architectures"] != model_arch:
                raise ValueError(
                    ...
                    f"not match current model_arch {model_arch}"
                )
            if cfg["num_layers"] != self.layer_num:
                raise ValueError(
                    f"num_layers {cfg['num_layers']} in config "
                    f"not match current layer_num {self.layer_num}"
                )
            if (
                cfg["num_head"] % self.total_head_num != 0
                and self.total_head_num % cfg["num_head"] != 0
            ):
                raise ValueError(
            ...
            if get_env_start_args().enable_fa3:
                if cfg["quant_type"] != "per_head":
                    raise ValueError(
                        f"quant type {cfg['num_head']} in config not match fa3 backend"
                    )
            else:
                if cfg["quant_type"] != "per_tensor":
                    raise ValueError(
                        f"quant type {cfg['quant_type']} in config not match flashinfer backend"
                    )
    ...
    def update_calibration_data(self, kv_buffer: torch.Tensor, layer_index: int):
        ...
        if (
            not get_model_init_status()
            or self.count >= warmup_counts + inference_counts
        ):
            return

        if self.count == 0 and layer_index == 0:
            logger.info(
                "kv cache calibration mode will collect kv cache data for quantization calibration"
            )
        ...
        if self.abs_max is not None and self.count >= warmup_counts:
            if get_env_start_args().enable_fa3:
                ...
                kv_max = torch.tensor(
                    [k_max, v_max], device="cuda", dtype=torch.float32
                )
            self.abs_max[layer_index] = torch.maximum(self.abs_max[layer_index], kv_max)
            if (
                self.count == warmup_counts + inference_counts - 1
                and layer_index == self.layer_num - 1
            ):
                ...
                if dist.is_initialized() and dist.get_world_size() > 1:
                    if get_env_start_args().enable_fa3:
                        ...
                        gathered_k_max = [
                            torch.zeros_like(k_max)
                            for _ in range(dist.get_world_size())
                        ]
                        gathered_v_max = [
                            torch.zeros_like(v_max)
                            for _ in range(dist.get_world_size())
                        ]
                        dist.all_gather(
                            gathered_k_max, k_max, group=None, async_op=False
                        )
                        dist.all_gather(
                            gathered_v_max, v_max, group=None, async_op=False
                        )
                ...
                self.scales = torch.where(
                    self.scales > 0, self.scales, torch.ones_like(self.scales)
                )
    ...
    def _export_calibration_data(self):
        ...
        cfg = {
            ...
            "quant_type": (
                "per_head" if get_env_start_args().enable_fa3 else "per_tensor"
            ),
            "qmin": self.qmin,
            "qmax": self.qmax,
