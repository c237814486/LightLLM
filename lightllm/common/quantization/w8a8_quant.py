...
class BaseQuantizationMethod(QuantizationMethod):
    def __init__(self):
        ...
        from lightllm.common.basemodel.layer_infer.cache_tensor_manager import (
            g_cache_manager,
        )
...
class FP8w8a8QuantizationMethod(BaseQuantizationMethod):
    ...
    def quantize(self, weight: torch.Tensor):
        ...
        qweight, weight_scale = scaled_fp8_quant(
            weight.contiguous().cuda(self.device_id_),
            scale=None,
...
class FP8w8a8B128QuantizationMethod(BaseQuantizationMethod):
    ...
    def apply(
        ...
        if input_scale is None:
            qinput_tensor, input_scale = per_token_group_quant_fp8(input_tensor, self.block_size, dtype=qweight.dtype, alloc_func=alloc_func)
        if out is None:
