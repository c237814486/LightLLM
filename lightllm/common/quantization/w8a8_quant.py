    ...
    def apply(self, input_tensor, weights, bias=None, out=None, workspace=None, use_custom_tensor_mananger=True):
        ...
        if out is None:
            if use_custom_tensor_mananger:
                out = self.cache_manager.alloc_tensor((m, n), input_tensor.dtype, device=input_tensor.device, is_graph_out=False)
            else:
...
class FP8w8a8QuantizationMethod(BaseQuantizationMethod):
    ...
    def quantize(self, weight: torch.Tensor):
        ...
        qweight, weight_scale = scaled_fp8_quant(weight.contiguous().cuda(self.device_id_), scale=None, use_per_token_if_dynamic=True)
    ...
    def quantize_moe(self, weight):
        ...
        for i in range(num_experts):
            qweight, weight_scale = scaled_fp8_quant(weight[i].contiguous().cuda(self.device_id_), scale=None, use_per_token_if_dynamic=True)
            qweights[i] = qweight
...
class FP8w8a8B128QuantizationMethod(BaseQuantizationMethod):
    ...
    def apply(self, input_tensor, weights, bias=None, out=None, workspace=None, use_custom_tensor_mananger=True):
        ...
        if input_scale is None:
            input_scale = self.cache_manager.alloc_tensor((m, k // self.block_size), torch.float32, device=input_tensor.device, is_graph_out=False)
