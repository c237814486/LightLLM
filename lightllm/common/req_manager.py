...
class ReqManager:
    def __init__(
        self, max_request_num, max_sequence_length, mem_manager: MemoryManager
    ):
...
class ReqSamplingParamsManager:
    ...
    def __init__(self, max_request_num):
        ...
        self.req_to_presence_penalty = torch.zeros(
            max_request_num + 1, dtype=torch.float32, device="cuda"
        )
        self.req_to_frequency_penalty = torch.zeros(
            max_request_num + 1, dtype=torch.float32, device="cuda"
        )
        self.req_to_repetition_penalty = torch.zeros(
            max_request_num + 1, dtype=torch.float32, device="cuda"
        )
    ...
    def init_req_sampling_params(self, req):
        ...
        self.req_to_exponential_decay_length_penalty[req.req_idx].fill_(
        ...
        if self.penalty_counter_mode == "cpu_counter":
            if (
                req.sampling_param.shm_param.input_penalty
                and req.need_out_token_id_statistics
            ):
                req.out_token_id_count = collections.Counter(
                    req.shm_req.get_prompt_ids()
                )
            else:
            ...
            if (
                ...
                prompt_ids = (
                    torch.from_numpy(req.shm_req.get_prompt_ids_numpy())
                    .pin_memory()
    ...
    def update_reqs_out_token_counter_gpu(
        ...
        assert (
            b_req_idx.is_cuda
            and next_token_ids.is_cuda
            and b_req_idx.shape[0] == next_token_ids.shape[0]
        )
