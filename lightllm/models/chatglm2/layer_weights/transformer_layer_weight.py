...
class ChatGLM2TransformerLayerWeight(LlamaTransformerLayerWeight):
    ...
    def _preprocess_weight(self, weights):
        ...
        if qkv_weight_name in weights:
            ...
            weights[self._k_weight_name] = qkv_weight_[
                self.n_embed : self.n_embed + n_kv_embed, :
            ]
            weights[self._v_weight_name] = qkv_weight_[
                self.n_embed + n_kv_embed : self.n_embed + 2 * n_kv_embed, :
            ]
        ...
        if qkv_bias_name in weights:
            ...
            weights[self._k_bias_name] = qkv_bias_[
                self.n_embed : self.n_embed + n_kv_embed
            ]
            weights[self._v_bias_name] = qkv_bias_[
                self.n_embed + n_kv_embed : self.n_embed + 2 * n_kv_embed
            ]
            del weights[qkv_bias_name]

        gate_up_weight_name = (
            f"transformer.encoder.layers.{self.layer_num_}.mlp.dense_h_to_4h.weight"
        )
        if gate_up_weight_name in weights:
            ...
            weights[self._up_weight_name] = gate_up_weight_[
                self.n_inter : 2 * self.n_inter, :
            ]
    ...
    def _init_weight_names(self):
        self._q_weight_name = (
            f"transformer.encoder.layers.{self.layer_num_}.self_attention.q_proj.weight"
        )
        self._q_bias_name = (
            f"transformer.encoder.layers.{self.layer_num_}.self_attention.q_proj.bias"
        )
        self._k_weight_name = (
            f"transformer.encoder.layers.{self.layer_num_}.self_attention.k_proj.weight"
        )
        self._k_bias_name = (
            f"transformer.encoder.layers.{self.layer_num_}.self_attention.k_proj.bias"
        )
        self._v_weight_name = (
            f"transformer.encoder.layers.{self.layer_num_}.self_attention.v_proj.weight"
        )
        self._v_bias_name = (
            f"transformer.encoder.layers.{self.layer_num_}.self_attention.v_proj.bias"
        )
        self._o_weight_name = (
            f"transformer.encoder.layers.{self.layer_num_}.self_attention.dense.weight"
        )
        self._o_bias_name = None

        self._gate_weight_name = (
            f"transformer.encoder.layers.{self.layer_num_}.mlp.gate_proj.weight"
        )
        self._gate_bias_name = None
        self._up_weight_name = (
            f"transformer.encoder.layers.{self.layer_num_}.mlp.up_proj.weight"
        )
        self._up_bias_name = None
        self._down_weight_name = (
            f"transformer.encoder.layers.{self.layer_num_}.mlp.dense_4h_to_h.weight"
        )
        self._down_bias_name = None

        self._att_norm_weight_name = (
            f"transformer.encoder.layers.{self.layer_num_}.input_layernorm.weight"
        )
