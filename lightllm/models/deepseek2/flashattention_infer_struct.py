        ...
        if self.is_prefill:
            ...
            if self.has_prefix_kv:
                self.cu_seqlens_prefix_k = torch.nn.functional.pad(
                    torch.cumsum(self.b_ready_cache_len, dim=0, dtype=torch.int32),
                    (1, 0),
                )
            ...
            if self.batch_size <= model.graph_max_batch_size and self.max_len_in_batch <= model.graph_max_len_in_batch:
                ...
                self.page_table = page_buffer[self.microbatch_index][: att_batch_size * model.graph_max_len_in_batch].view(att_batch_size, model.graph_max_len_in_batch)
            else:
                self.page_table = torch.empty((att_batch_size, self.max_len_in_batch), dtype=torch.int32).to(input_ids.device)
            page_table_copy(
