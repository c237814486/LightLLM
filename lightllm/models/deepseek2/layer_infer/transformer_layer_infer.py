...
from lightllm.models.deepseek2.triton_kernel.context_flashattention_nopad_fp8 import (
...
from lightllm.utils.sgl_utils import (
...
class Deepseek2TransformerLayerInfer(LlamaTransformerLayerInfer):
    def __init__(self, layer_num, network_config, mode=[]):
        ...
        self.is_moe = network_config["n_routed_experts"] is not None and layer_num >= network_config["first_k_dense_replace"] and layer_num % network_config["moe_layer_freq"] == 0
        ...
        self.enable_cc_method = not os.getenv("DISABLE_CC_METHOD", "False").upper() in [
            "ON",
            "TRUE",
            "1",
        ]
    ...
    def _bind_attention(self):
        if "triton_fp8kv" in self.mode:
            ...
            self._token_attention_kernel = partial(
                Deepseek2TransformerLayerInfer._token_gqa_decode_attention_flashdecoding_fp8,
                self,
            )
        else:
            ...
            if get_env_start_args().enable_fa3:
                self._token_attention_kernel = partial(
                    Deepseek2TransformerLayerInfer._token_gqa_decode_attention_flashattention,
                    self,
                )
            elif get_env_start_args().enable_flashinfer_decode:
                self._token_attention_kernel = partial(
                    Deepseek2TransformerLayerInfer._token_gqa_decode_attention_flashinfer,
                    self,
                )
            else:
                self._token_attention_kernel = partial(
                    Deepseek2TransformerLayerInfer._token_gqa_decode_attention_flashdecoding,
                    self,
                )
            ...
            if "triton_fp8kv" in self.mode:
                if get_env_start_args().enable_flashinfer_prefill:
                    self._context_attention_kernel = partial(
                        Deepseek2TransformerLayerInfer._context_attention_flashinfer_kernel_with_CC_fp8,
                        self,
                    )
                else:
                    self._context_attention_kernel = partial(
                        Deepseek2TransformerLayerInfer._context_attention_kernel_with_CC_fp8,
                        self,
                    )
            else:
                if get_env_start_args().enable_fa3:
                    self._context_attention_kernel = partial(
                        Deepseek2TransformerLayerInfer._context_attention_flashattention_kernel_with_CC,
                        self,
                    )
                elif get_env_start_args().enable_flashinfer_prefill:
                    self._context_attention_kernel = partial(
                        Deepseek2TransformerLayerInfer._context_attention_flashinfer_kernel_with_CC,
                        self,
                    )
                else:
                    self._context_attention_kernel = partial(
                        Deepseek2TransformerLayerInfer._context_attention_kernel_with_CC,
                        self,
                    )
        else:
            if "triton_fp8kv" in self.mode:
                self._context_attention_kernel = partial(
                    Deepseek2TransformerLayerInfer._context_attention_kernel_origin_fp8,
                    self,
                )
            else:
                self._context_attention_kernel = partial(
                    Deepseek2TransformerLayerInfer._context_attention_kernel_origin,
                    self,
                )

    def _pre_cache_kv(self, infer_state: Deepseek2InferStateInfo, layer_weight: Deepseek2TransformerLayerWeight) -> torch.Tensor:
    ...
    def _get_qkv(
        ...
        if self.q_lora_rank is None:
            ...
            q, cache_kv = layer_weight.qkv_a_proj_with_mqa_.mm(input).split([self.q_lora_rank, self.kv_lora_rank + self.qk_rope_head_dim], dim=-1)
    ...
    def _tpsp_get_qkv(self, input, infer_state: Deepseek2InferStateInfo, layer_weight: Deepseek2TransformerLayerWeight) -> torch.Tensor:
        input = input.view(-1, self.embed_dim_)
        if self.q_lora_rank is None:
            ...
            if self.tp_world_size_ > 1:
                ...
                gather_qkv = self.alloc_tensor((sp_token_num * self.tp_world_size_, qkv_dim), dtype=qkv.dtype, device=qkv.device)
    ...
    def _decompress_kv(
        ...
        if not skip_sample:
            ...
            compressed_kv, k_rope = torch.split(kv, [layer_weight.kv_lora_rank, layer_weight.qk_rope_head_dim], dim=-1)  # (b*s, 1, kv_lora + qk_r)
    ...
    def _context_attention_flashattention_kernel_with_CC(
        ...
        k = torch.cat(
            [k_nope, torch.repeat_interleave(k_rope, self.tp_q_head_num_, dim=-2)],
            dim=-1,
        )
        ...
        if infer_state.has_prefix_kv:
            ...
            tmp_output = self.alloc_tensor((q.shape[0], q.shape[1], self.qk_nope_head_dim), dtype=q.dtype) if out is None else out
    ...
    def _context_attention_flashinfer_kernel_with_CC(
        ...
        o_tensor = self.alloc_tensor((q.shape[0], q.shape[1], self.qk_nope_head_dim), dtype=q.dtype) if out is None else out
        k = torch.cat(
            [k_nope, torch.repeat_interleave(k_rope, self.tp_q_head_num_, dim=-2)],
            dim=-1,
        )
    ...
    def _tpsp_ffn_tp(self, input, infer_state: Deepseek2InferStateInfo, layer_weight: Deepseek2TransformerLayerWeight) -> torch.Tensor:
        input = input.view(-1, self.embed_dim_)
        if self.tp_world_size_ > 1:
            sp_token_num, hidden_dim = input.shape
            gather_input = self.alloc_tensor((sp_token_num * self.tp_world_size_, hidden_dim), dtype=input.dtype, device=input.device)
    ...
    def _tpsp_ffn_ep(self, input, infer_state: Deepseek2InferStateInfo, layer_weight: Deepseek2TransformerLayerWeight) -> torch.Tensor:
        input = input.view(-1, self.embed_dim_)
    ...
    def overlap_tpsp_token_forward(
        ...
        if not self.is_moe:
            return super().overlap_tpsp_token_forward(input_embdings, input_embdings1, infer_state, infer_state1, layer_weight)
        # 0 attention
        ...
        expected_m = triton.cdiv(
            input_embdings.shape[0] * get_global_world_size() * self.num_experts_per_tok,
        ...
        # 0 combine
        _0_ffn_out, _0_hook = layer_weight.experts.low_latency_combine(_0_moe_out, _0_topk_idx, _0_topk_weight, _0_handle)
        ...
        # 1 combine
        _1_ffn_out, _1_hook = layer_weight.experts.low_latency_combine(_1_moe_out, _1_topk_idx, _1_topk_weight, _1_handle)
    ...
    def overlap_tpsp_context_forward(
        ...
        if not self.is_moe:
            return super().overlap_tpsp_context_forward(input_embdings, input_embdings1, infer_state, infer_state1, layer_weight)
        # 0 attention
        _0_input1 = self._att_norm(input_embdings, infer_state, layer_weight)
        _0_q, _0_cache_kv = self._tpsp_get_qkv(_0_input1, infer_state, layer_weight)
        _0_input1 = None
        ...
        _0_topk_weight, _0_topk_idx, _0_qinput_tensor = layer_weight.experts.select_experts_and_quant_input(_0_input1, _0_router_logits)
        ...
        # 0 dispatch execute
        (_0_recv_x, _0_recv_topk_idx, _0_recv_topk_weight, _0_num_recv_tokens_per_expert_list, _0_handle, _0_hook,) = layer_weight.experts.dispatch(
            _0_qinput_tensor,
            _0_topk_idx,
            _0_topk_weight,
        ...
        _1_topk_weight, _1_topk_idx, _1_qinput_tensor = layer_weight.experts.select_experts_and_quant_input(_1_input1, _1_router_logits)
        ...
        # 1 dispatch execute
        (_1_recv_x, _1_recv_topk_idx, _1_recv_topk_weight, _1_num_recv_tokens_per_expert_list, _1_handle, _1_hook,) = layer_weight.experts.dispatch(
            _1_qinput_tensor,
            _1_topk_idx,
            _1_topk_weight,
