...
class Deepseek2TransformerLayerWeight(TransformerLayerWeight):
    ...
    def _parse_config(self):
        super()._parse_config()
        self.is_moe = (
            self.network_config_["n_routed_experts"] is not None and self.layer_num_ >= self.network_config_["first_k_dense_replace"] and self.layer_num_ % self.network_config_["moe_layer_freq"] == 0
        )
    ...
    def _load_q_rope(self, q_weight_):
        q_split_n_embed_with_rope = (self.qk_nope_head_dim + self.qk_rope_head_dim) * self.num_attention_heads // self.tp_world_size_
        q_weight_ = q_weight_[
    ...
    def _load_kb(self, kv_b_proj_):
        k_b_proj_ = kv_b_proj_.view(self.num_attention_heads, self.qk_nope_head_dim * 2, self.kv_lora_rank)[:, : self.qk_nope_head_dim, :]
    ...
    def _load_vb_scale(self, kv_b_proj_scale_, block_size):
        v_b_proj_scale_ = kv_b_proj_scale_.T.view(self.kv_lora_rank // block_size, self.num_attention_heads, self.qk_nope_head_dim * 2 // block_size,)[
            :, :, self.qk_nope_head_dim // block_size :
        ].transpose(0, 1)
    ...
    def load_hf_weights(self, weights):
        ...
        if self.quant_cfg.quantized_weight and f"model.layers.{self.layer_num_}.self_attn.kv_b_proj." + kv_b_quant_method.weight_scale_suffix in weights:
            kv_b_proj_scale_ = weights[f"model.layers.{self.layer_num_}.self_attn.kv_b_proj." + kv_b_quant_method.weight_scale_suffix]
            block_size = 128
            weights[f"model.layers.{self.layer_num_}.self_attn.k_b_proj." + kv_b_quant_method.weight_scale_suffix] = self._load_kb_scale(kv_b_proj_scale_, block_size)
            weights[f"model.layers.{self.layer_num_}.self_attn.v_b_proj." + kv_b_quant_method.weight_scale_suffix] = self._load_vb_scale(kv_b_proj_scale_, block_size)
    ...
    def _init_norm(self):
        ...
        self.kv_a_layernorm_ = NormWeight(
            f"model.layers.{self.layer_num_}.self_attn.kv_a_layernorm.weight",
            self.data_type_,
        )
        if self.q_lora_rank is not None:
            self.q_a_layernorm_ = NormWeight(
                f"model.layers.{self.layer_num_}.self_attn.q_a_layernorm.weight",
                self.data_type_,
            )
