...
class Deepseek2TransformerLayerWeight(TransformerLayerWeight):
    ...
    def load_hf_weights(self, weights):
        ...
        if self.quant_cfg.quantized_weight and f"model.layers.{self.layer_num_}.self_attn.kv_b_proj." + weight_scale_suffix in weights:
            kv_b_proj_scale_ = weights[f"model.layers.{self.layer_num_}.self_attn.kv_b_proj." + weight_scale_suffix]
            block_size = 128
            weights[f"model.layers.{self.layer_num_}.self_attn.k_b_proj." + weight_scale_suffix] = self._load_kb_scale(kv_b_proj_scale_, block_size)
            weights[f"model.layers.{self.layer_num_}.self_attn.v_b_proj." + weight_scale_suffix] = self._load_vb_scale(kv_b_proj_scale_, block_size)
