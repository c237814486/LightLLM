...
class Deepseek3MTPPreLayerInfer(LlamaPreLayerInfer):
    ...
    def _mtp_context_forward(
        ...
        layer_weight: Deepseek3MTPPreAndPostLayerWeight,
    ):
        tgt_embdings = infer_state.deepseekv3_mtp_draft_input_hiddens
        assert input_embdings.shape[0] == tgt_embdings.shape[0]
        rmsnorm_forward(
            input_embdings,
            weight=layer_weight.enorm_weight_,
            eps=self.eps_,
            out=input_embdings,
        )
        rmsnorm_forward(
            tgt_embdings,
            weight=layer_weight.hnorm_weight_,
            eps=self.eps_,
            out=tgt_embdings,
        )
        ...
        ans_logics = self.alloc_tensor(
            (cat_embdings.shape[0], layer_weight.eh_proj_weight_.shape[1]),
