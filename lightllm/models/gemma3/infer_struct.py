...
class Gemma3InferStateInfo(InferStateInfo):
    ...
    def init_some_extra_state(self, model, input_ids: torch.Tensor):
        ...
        if self.is_prefill:
            ...
            self.position_cos_local = torch.index_select(
                model._cos_cached_local, 0, position_ids
            ).view(position_ids.shape[0], -1)
            self.position_sin_local = torch.index_select(
                model._sin_cached_local, 0, position_ids
            ).view(position_ids.shape[0], -1)

            self.position_cos_global = torch.index_select(
                model._cos_cached_global, 0, position_ids
            ).view(position_ids.shape[0], -1)
            self.position_sin_global = torch.index_select(
                model._sin_cached_global, 0, position_ids
            ).view(position_ids.shape[0], -1)
        else:
            position_ids = self.position_ids
            self.position_cos = torch.index_select(
