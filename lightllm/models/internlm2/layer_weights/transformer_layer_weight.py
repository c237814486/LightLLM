...
class Internlm2TransformerLayerWeight(LlamaTransformerLayerWeight):
    ...
    def load_hf_weights(self, weights):
        ...
        if qkv_weight_name in weights:
            ...
            qkv_weight_ = qkv_weight_.reshape(
                self.n_kv_head, q_groups + 2, self.head_dim, -1
            )
            q_weight_ = qkv_weight_[:, :q_groups, :, :].reshape(
                -1, qkv_weight_.shape[-1]
            )
    ...
    def _init_weight_names(self):
        ...
        self._gate_weight_name = (
            f"model.layers.{self.layer_num_}.feed_forward.w1.weight"
        )
        self._up_weight_name = f"model.layers.{self.layer_num_}.feed_forward.w3.weight"
        self._down_weight_name = (
            f"model.layers.{self.layer_num_}.feed_forward.w2.weight"
        )
        self._att_norm_weight_name = (
            f"model.layers.{self.layer_num_}.attention_norm.weight"
        )
