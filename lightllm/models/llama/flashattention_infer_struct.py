    ...
    def _init_flash_attention_state(self, model, input_ids: torch.Tensor):
        if self.is_prefill:
            ...
            if self.batch_size <= model.graph_max_batch_size and self.max_len_in_batch <= model.graph_max_len_in_batch:
                page_buffer = FlashAttentionStateInfo.get_page_table_buffer(model.graph_max_batch_size, model.graph_max_len_in_batch)
        ...
        if "offline_calibration_fp8kv" in model.mode:
            if self.is_prefill:
                ...
                self.q_scale = torch.empty(
                    (self.batch_size, self.mem_manager.head_num),
                    dtype=torch.float32,
                    device=device,
                )
                self.token_batch_ids = torch.repeat_interleave(torch.arange(self.batch_size, device=device), self.b_q_seq_len)

            offline_scales = self.mem_manager.scales
            head_num = self.mem_manager.head_num
            # 为了减少推理计算量，在推理外部初始化k_descale和v_descale
            self.k_descale = (
                offline_scales[:, :head_num].view(-1, 1, head_num).expand(offline_scales.shape[0], self.batch_size, head_num)
