...
class LlamaFlashInferStateExtraInfo:
    def __init__(self, model):
        ...
        self.tp_kv_head_num = max(
            model.config["num_key_value_heads"] // tp_world_size, 1
        )
        head_dim = model.config["hidden_size"] // model.config["num_attention_heads"]
        self.head_dim = model.config.get("head_dim", head_dim)
        self.workspace_buffer = torch.empty(
            512 * 1024 * 1024, dtype=torch.int8, device=get_current_device_id()
        )
...
class LlamaTpPartModel(TpPartBaseModel):
    ...
    def _verify_params(self):
        assert self.load_way in [
            "HF",
            "DS",
        ], "llama only supports HF and DS format to load Now!"
    ...
    def _init_to_get_rotary(self, default_base=10000):
        ...
        t = (
            torch.arange(
                max(max_seq_len + 1024 * 128, self.max_seq_length),
                device="cpu",
                dtype=torch.float32,
            )
            / rope_scaling_factor
    ...
    def _init_to_get_dynamic_ntk_rotary(self):
        ...
        self._cos_cached = torch.zeros(
            (max_seq_len, partial_head_dim // 2), dtype=self.data_type, device="cuda"
        )
        self._sin_cached = torch.zeros(
            (max_seq_len, partial_head_dim // 2), dtype=self.data_type, device="cuda"
        )
        ...
        for seq_loc_index in range(max_position_embeddings, max_seq_len, 1):
            ...
            inv_freq = 1.0 / (
                ...
                ** (
                    torch.arange(
                        0, partial_head_dim, 2, device="cpu", dtype=torch.float32
                    )
                    / partial_head_dim
                )
            )
            ...
            freqs = torch.outer(t, inv_freq)
            self._cos_cached[seq_loc_index : seq_loc_index + 1, :] = (
                torch.cos(freqs).to(self.data_type).cuda()
            )
            self._sin_cached[seq_loc_index : seq_loc_index + 1, :] = (
                torch.sin(freqs).to(self.data_type).cuda()
            )
        return

    def _init_to_get_yarn_rotary(self):
        from .yarn_rotary_utils import (
        ...
        mscale = float(
            get_mscale(scale) * attn_factor
        )  # Get n-d magnitude scaling corrected for interpolation

        # Build here to make `torch.jit.trace` work.
        max_seq_len_cached = max_position_embeddings
        t = torch.arange(
            max(max_seq_len_cached, self.max_seq_length),
            device="cuda",
            dtype=torch.float32,
        )
    ...
    def _init_to_su_rotary(self):
        ...
        original_max_position_embeddings = self.config[
        ...
        if scale <= 1.0:
            ...
            rope_scaling_factor = math.sqrt(
                1 + math.log(scale) / math.log(original_max_position_embeddings)
            )

        max_seq_len = max(self.max_seq_length, max_position_embeddings)
        self._cos_cached = torch.zeros(
            (max_seq_len, self.head_dim_ // 2), dtype=self.data_type, device="cuda"
        )
        self._sin_cached = torch.zeros(
            (max_seq_len, self.head_dim_ // 2), dtype=self.data_type, device="cuda"
        )
        ...
        t = torch.arange(
            original_max_position_embeddings, device="cpu", dtype=torch.float32
        )
        freqs = torch.outer(t, inv_freq)
    ...
    def _init_to_get_llama3_rotary(self, default_base=10000):
        ...
        low_freq_factor = self.config.get("rope_scaling", {}).get(
            "low_freq_factor", 1.0
        )
        high_freq_factor = self.config.get("rope_scaling", {}).get(
            "high_freq_factor", 4.0
        )
        origin_context_len = self.config.get("rope_scaling", {}).get(
        ...
        for freq in inv_freq:
            ...
            if wavelen < high_freq_wavelen:
                ...
                smooth = (origin_context_len / wavelen - low_freq_factor) / (
        ...
        t = torch.arange(
            max(max_seq_len, self.max_seq_length), device="cpu", dtype=torch.float32
        )
        freqs = torch.outer(t, inv_freq)
