...
class PoolPojector(nn.Module):
    ...
    def forward(self, x):
        x = rearrange(x, "b (h w) d -> b d h w", h=self.config["mm_image_size"]['height']//self.config["mm_patch_size"], w=self.config["mm_image_size"]['width']//self.config["mm_patch_size"])
...
class AudioAvgPoolProjector(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.config = config
        self.audio_avg_pooler = nn.AvgPool1d(self.config.audio_downsample_ratio, stride=self.config.audio_downsample_ratio)
        self.linear1 = nn.Linear(in_features=self.config.audio_hidden_size, out_features=self.config.hidden_size, bias=True)
        self.relu = nn.GELU() # or Silu？ minicpm用的relu
        self.linear2 = nn.Linear(in_features=self.config.hidden_size, out_features=self.config.hidden_size, bias=True)
