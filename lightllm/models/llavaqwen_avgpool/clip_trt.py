...
from transformers import (
    CLIPVisionModel as CLIPVisionModelHF,
...
class CLIPVisionModelTRT:
    ...
    def init_trt(
        self,
        trt_cache="/mnt/afs/yangdeyu/TTS/GameMLLM/LLaVA_hub/checkpoints/llava_2_20_ft_gpt_all",
        **kwargs,
    ):
        ...
        trt_path = os.path.join(
            os.path.expanduser(trt_cache),
            trt_model_filename(self.config.name, suffix=suffix),
        )
        ...
        if os.path.isfile(trt_path):
            ...
            # 定义动态维度范围
            dynamic_axes = {"input_0": [(0, 1, 128)]}  # 第0维(batch)可以在1到32之间变化
