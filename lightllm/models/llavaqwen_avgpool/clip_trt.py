...
from .trt_util import AttributeDict,  convert_tensor, trt_model_filename, torch_image


class CLIPVisionModelTRT():
    """
    ...
    def from_pretrained(model="openai/clip-vit-large-patch14-336", dtype=torch.float16, 
        ...
        """                
        if use_cache and model in CLIPVisionModelTRT.ModelCache:
            return CLIPVisionModelTRT.ModelCache[model]
            
        instance = CLIPVisionModelTRT(model, dtype=dtype, projector=projector, crop=crop, use_tensorrt=use_tensorrt, **kwargs)
        
        if use_cache:
            CLIPVisionModelTRT.ModelCache[model] = instance
    ...
    def __init__(self, model, dtype=torch.float16, projector=None, crop=None, use_tensorrt=True, **kwargs):
        ...
        self.config.input_shape = (self.model.config.image_size["height"], self.model.config.image_size["width"])
        ...
        print("image size", self.config.input_shape)
        self.forward(PIL.Image.new('RGB', self.config.input_shape, (255,255,255)))
    ...
    def init_trt(self, trt_cache="/mnt/afs/yangdeyu/TTS/GameMLLM/LLaVA_hub/checkpoints/llava_2_20_ft_gpt_all", **kwargs): 
        ...
        if os.path.isfile(trt_path):
            ...
            # 定义动态维度范围
            dynamic_axes = {
                'input_0': [(0, 1, 128)]  # 第0维(batch)可以在1到32之间变化
            }
