...
ne.set_num_threads(min(os.cpu_count()//4, 24))  # 动态计算线程数
...
def batch_center_crop(
    ...
    # For batch processing, we assume the first dimension is always batch size, and input data format is always channel.first
    batch_size = image.shape[0]
    
    input_data_format = (ChannelDimension.FIRST if image.shape[1] in [1, 3, 4] 
    ...
    if top >= 0 and bottom <= orig_height and left >= 0 and right <= orig_width:
        ...
        cropped_images = new_images[:, :, max(0, top):min(new_height, bottom), max(0, left):min(new_width, right)]
...
def batch_normalize_numexpr(
    ...
    # 使用 numexpr 进行优化计算
    result = ne.evaluate('(image - mean) / std', local_dict={
        'image': image,
        'mean': mean,
        'std': std
    })
...
class OpimizedCLIPImageProcessor(CLIPImageProcessor):
    def __init__(self, do_resize: bool = True, 
                 ...
                 **kwargs) -> None:
        super().__init__(do_resize, size, resample, do_center_crop, crop_size, do_rescale, rescale_factor, do_normalize, image_mean, image_std, do_convert_rgb, **kwargs)
    ...
    def preprocess(
        ...
        **kwargs,
        ...
        input_data_format = (ChannelDimension.FIRST if images.shape[1] in [1, 3, 4] 
                            else ChannelDimension.LAST
                        )
        ...
        if do_normalize:
            image = batch_normalize_numexpr(image=image, mean=image_mean, std=image_std, input_data_format=input_data_format)
...
class Qwen25VLImageProcessorOptimized(OpimizedCLIPImageProcessor):
    ...
    def preprocess(self, images, 
        ...
        if do_normalize:
            images = batch_normalize_numexpr(image=images, mean=image_mean, std=image_std, input_data_format=input_data_format)
...
def compare_processors():
    ...
    for batch_size in batch_sizes:
        for height, width in image_sizes:
            ...
            qwen25_time_torch = run_performance_test(qwen25_processor, batch_size, height, width, num_runs=10, patch_reshape_method="torch")
            qwen25_time_numpy = run_performance_test(qwen25_processor, batch_size, height, width, num_runs=10, patch_reshape_method="numpy")
            ...
            # 输出结果
            print(f"{batch_size:^14} | {f'{height}x{width}':^16} | "
                  f"{original_time:^14.2f} | {optimized_time:^15.2f} | {qwen25_time_numpy:^15.2f} | {qwen25_time_torch:^15.2f} |"
                  f"{improvement:^15.1f}x | {max_diff:^10.8f}")
