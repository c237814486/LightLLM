...
from transformers.models.qwen2_5_vl.modeling_qwen2_5_vl import Qwen2_5_VisionTransformerPretrainedModel, Qwen2RMSNorm, Qwen2_5_VLVisionBlock, Qwen2_5_VisionPatchEmbed, apply_rotary_pos_emb_vision, apply_rotary_pos_emb_flashatt
...
class LlavaQwen25AvgpoolVisionModel:
    def __init__(self):
    ...
    def load_bin_model(self, config, weight_dir):
        ...
        self.vision_tower = Qwen25ViTPretrainedModel.from_pretrained(vision_path, ).to(torch.bfloat16) # 精度
    ...
    def forward(self, images, images_thw):
        ...
        image_embeds = self.vision_tower(
                images.to(device=torch.device("cuda"), dtype=torch.bfloat16),
                images_thw.to(device=torch.device("cuda"), dtype=torch.int32),
            )
        image_embeds = image_embeds.float()
        image_embeds, token_len_list = self.mm_projector(image_embeds, images_thw)
        
        B = int(len(token_len_list) * 2)
        token_num = token_len_list[0] // 2
        image_embeds = image_embeds.view(B, token_num, -1)
        # print(image_embeds.shape, token_len_list)
        return image_embeds
    ...
    def get_image_tensor(self, image_uuids):
        ...
        img_tensors, imgs_thw = self.process_image(batch_images)
        # print("image tensor", img_tensors.shape, "imgs thw", imgs_thw, "image_num", len(batch_images))
        
        cur_num = len(batch_images)
        valid_ids = [[i, i+1] for i in range(cur_num)]

        return img_tensors, imgs_thw, uuids, valid_ids
    ...
    def encode(self, image_uuids: List):
        ...
        logger.debug(f"[encode image] 推理耗时: {end - start:.4f} 秒 image num {len(valid_ids)}")
...
class FLOPCounter:
    ...
    def measure_vit_flops(self, input_tensor, images_thw):
        ...
        try:
            ...
            custom_rules = {
            Qwen2_5_VLVisionBlock: count_custom_module,
            Qwen2_5_VisionPatchEmbed: count_custom_module,
            Qwen2_5_VLPatchMerger: count_custom_module,
        }
            ...
            flops, params = profile(
                vit_model, 
                inputs=(images_input, images_thw_input), 
                custom_ops=custom_rules,
                verbose=True
            )
    ...
    def get_flops_breakdown(self, input_tensor, images_thw):
        ...
        def sum_module_flops(module):
            total_ops = reduce(operator.add, (
                getattr(d, 'total_ops', 0) for d in module.modules()
            ))
            ...
            for model_name, model in self.models_to_test.items():
                ...
                try:
                    ...
                    if bs == self.config["batch_sizes"][0]:
                        self.console.print(f"    -> 测量ViT FLOPs...")
...
if __name__ == "__main__":
    ...
    # --- 1. 配置中心 ---
    BENCHMARK_CONFIG = {
        "model_path": "/mnt/afs/yangdeyu/GameMLLM/LLaVA_hub/checkpoints/omni_models/0706_llava_omni_qwen25vl_14B_16x_8k_vitlora_kimiwhisper_10x_conv_channelupscale", 
    ...
    for i in range(10):
        _ = model_eager.forward(image_tensors, images_thw)
