    ...
    def init_some_extra_state(self, model, input_ids: torch.Tensor):
        ...
        if self.is_prefill:
            ...
            infer_ntk_id = torch.clamp(
                torch.ceil(
                    torch.log2(self.b_seq_len / model.config.get("seq_length", 2048))
                    + 1
                ),
                0,
                model.max_ntk_alpha,
            ...
            for i in range(len(infer_ntk_id)):
                ...
                self.position_sin.append(
                    model._sin_cached[infer_ntk_id[i]][position_ids[_start:_end]]
                )
                self.position_cos.append(
                    model._cos_cached[infer_ntk_id[i]][position_ids[_start:_end]]
                )
            ...
            if model.logn_tensor is not None:
                self.logn_values = torch.index_select(
                    model.logn_tensor, 0, position_ids
                ).view(-1)
            position_ids = None
        else:
            infer_ntk_id = torch.clamp(
            ...
            position_ids = self.position_ids
            self.position_cos = model._cos_cached[infer_ntk_id, position_ids].view(
                position_ids.shape[0], -1
            )
            self.position_sin = model._sin_cached[infer_ntk_id, position_ids].view(
                position_ids.shape[0], -1
            )
            if model.logn_tensor is not None:
                self.logn_values = torch.index_select(
