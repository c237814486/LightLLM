...
from lightllm.models.internvl.img_process import (
    get_image_patch as default_get_image_patch,
)
...
def get_load_image_func(weight_dir):
    ...
    if os.path.exists(pre_process_path):
        logger.info(
            f"Found pre_process.py in {weight_dir}, attempting to load load_image from it."
        )
        ...
        logger.info(
            f"pre_process.py not found in {weight_dir}, using default load_image."
        )
...
def get_image_patch_func(weight_dir):
    ...
    if os.path.exists(pre_process_path):
        logger.info(
            f"Found pre_process.py in {weight_dir}, attempting to load get_image_patch from it."
        )
        spec = importlib.util.spec_from_file_location("pre_process", pre_process_path)
        pre_process = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(pre_process)
        ...
        logger.info(
            f"pre_process.py not found in {weight_dir}, using default get_image_patch."
        )
