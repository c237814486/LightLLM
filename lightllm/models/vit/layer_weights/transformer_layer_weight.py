...
class ViTTransformerLayerWeight(TransformerLayerWeight):
    ...
    def _init_weight_names(self):
        self._att_norm_weight_name = (
            f"vision_model.encoder.layers.{self.layer_num_}.norm1.weight"
        )

        self._q_weight_name = (
            f"vision_model.encoder.layers.{self.layer_num_}.attn.q.weight"
        )
        self._k_weight_name = (
            f"vision_model.encoder.layers.{self.layer_num_}.attn.k.weight"
        )
        self._v_weight_name = (
            f"vision_model.encoder.layers.{self.layer_num_}.attn.v.weight"
        )

        if self.qkv_bias:
            self._q_bias_name = (
                f"vision_model.encoder.layers.{self.layer_num_}.attn.q.bias"
            )
            self._k_bias_name = (
                f"vision_model.encoder.layers.{self.layer_num_}.attn.k.bias"
            )
            self._v_bias_name = (
                f"vision_model.encoder.layers.{self.layer_num_}.attn.v.bias"
            )
        else:
        ...
        self._o_weight_name = (
            f"vision_model.encoder.layers.{self.layer_num_}.attn.proj.weight"
        )
        self._o_bias_name = (
            f"vision_model.encoder.layers.{self.layer_num_}.attn.proj.bias"
        )

        self.fc1_weight_name_ = (
            f"vision_model.encoder.layers.{self.layer_num_}.mlp.fc1.weight"
        )
        self.fc1_bias_name_ = (
            f"vision_model.encoder.layers.{self.layer_num_}.mlp.fc1.bias"
        )
        self.fc2_weight_name_ = (
            f"vision_model.encoder.layers.{self.layer_num_}.mlp.fc2.weight"
        )
        self.fc2_bias_name_ = (
            f"vision_model.encoder.layers.{self.layer_num_}.mlp.fc2.bias"
        )
        ...
        self._ffn_norm_weight_name = (
            f"vision_model.encoder.layers.{self.layer_num_}.norm2.weight"
        )

        if self.norm_type == "layer_norm":
            self._att_norm_bias_name = (
                f"vision_model.encoder.layers.{self.layer_num_}.norm1.bias"
            )
            self._ffn_norm_bias_name = (
                f"vision_model.encoder.layers.{self.layer_num_}.norm2.bias"
            )
        else:
        ...
        if self.qk_norm:
            self._q_norm_weight_name = (
                f"vision_model.encoder.layers.{self.layer_num_}.attn.q_norm.weight"
            )
            self._k_norm_weight_name = (
                f"vision_model.encoder.layers.{self.layer_num_}.attn.k_norm.weight"
            )
    ...
    def _init_norm(self):
        ...
        if self.qk_norm:
            ...
            self.q_norm_weight_ = TpNormWeight(
                self._q_norm_weight_name, self.data_type_, split_n_embed
            )
            self.k_norm_weight_ = TpNormWeight(
                self._k_norm_weight_name, self.data_type_, split_n_embed
            )
    ...
    def load_hf_weights(self, weights):
        if f"vision_model.encoder.layers.{self.layer_num_}.attn.qkv.weight" in weights:
            ...
            att_qkv_dense_weight = weights[
                f"vision_model.encoder.layers.{self.layer_num_}.attn.qkv.weight"
            ]
            att_qkv_dense_weight = att_qkv_dense_weight.reshape(3, n_embed, -1)
            q_weight_ = F.pad(
                att_qkv_dense_weight[0, :, :], (0, 0, 0, self.padding_hidden_size)
            )
            k_weight_ = F.pad(
                att_qkv_dense_weight[1, :, :], (0, 0, 0, self.padding_hidden_size)
            )
            v_weight_ = F.pad(
                att_qkv_dense_weight[2, :, :], (0, 0, 0, self.padding_hidden_size)
            )
            del weights[
                f"vision_model.encoder.layers.{self.layer_num_}.attn.qkv.weight"
            ]
        ...
        if self._o_weight_name in weights:
            weights[self._o_weight_name] = F.pad(
                weights[self._o_weight_name], (0, self.padding_hidden_size, 0, 0)
            )

        if f"vision_model.encoder.layers.{self.layer_num_}.attn.qkv.bias" in weights:
            n_embed = self.network_config_["hidden_size"]
            att_qkv_dense_bias = weights[
                f"vision_model.encoder.layers.{self.layer_num_}.attn.qkv.bias"
            ]
            att_qkv_dense_bias = F.pad(
                att_qkv_dense_bias, (0, self.padding_hidden_size)
            ).reshape(3, -1)
        ...
        if self.qk_norm and self._q_norm_weight_name in weights:
            weights[self._q_norm_weight_name] = F.pad(
                weights[self._q_norm_weight_name], (0, self.padding_hidden_size)
            )

        if self.qk_norm and self._k_norm_weight_name in weights:
            weights[self._k_norm_weight_name] = F.pad(
                weights[self._k_norm_weight_name], (0, self.padding_hidden_size)
            )
