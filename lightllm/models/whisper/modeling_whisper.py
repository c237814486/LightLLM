...
def _compute_mask_indices(
    ...
    if mask_length > sequence_length:
        raise ValueError(f"`mask_length` has to be smaller than `sequence_length`, but got `mask_length`: {mask_length}" f" and `sequence_length`: {sequence_length}`")
    ...
    def forward(
        ...
        if layer_head_mask is not None:
            if layer_head_mask.size() != (self.num_heads,):
                raise ValueError(f"Head mask for a single layer should be of size {(self.num_heads,)}, but is" f" {layer_head_mask.size()}")
...
WHISPER_INPUTS_DOCSTRING = r"""
    Args:
        ...
        decoder_inputs_embeds:
            (`torch.FloatTensor` of shape `(batch_size, target_sequence_length, hidden_size)`, *optional*)
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                if self.gradient_checkpointing and self.training and (self.gradient_checkpointing_first_layer == -1 or idx <= self.gradient_checkpointing_first_layer):
    ...
    def forward(
        ...
        Args:
            ...
            encoder_hidden_states
                (`torch.FloatTensor` of shape `(batch_size, encoder_sequence_length, hidden_size)`, *optional*)
        ...
        for attn_mask, mask_name in zip([head_mask, cross_attn_head_mask], ["head_mask", "cross_attn_head_mask"]):
            if attn_mask is not None:
                assert attn_mask.size()[0] == (len(self.layers)), f"The `{mask_name}` should be specified for {len(self.layers)} layers, but it is for" f" {head_mask.size()[0]}."
