...
class DecodeReq:
    ...
    def stop_sequences_str_match(self) -> bool:
        ...
        for stop_str in stop_strs:
            if stop_str in tail_str:
                logger.debug(f"req_id {self.request_id} Found stop sequence in tail: stop_str='{stop_str}', " f"tail_str='{tail_str}'")
                return True
        return False

    def need_detoken(self):
        if (not self.req.is_aborted) and (not self.req.stop_str_matched) and len(self.output_ids) < self.req.candetoken_out_len:
            return True
        return False
    ...
    def can_set_release_mark(self):
        ...
        if self.req.stop_str_matched:
            return True
        if self.req.finish_status.is_finished() and self.req.candetoken_out_len == len(self.output_ids) and self.req.finish_token_index == self.input_len + len(self.output_ids) - 1:
            return True
        return False
