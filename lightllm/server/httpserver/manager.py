    ...
    async def generate(
        ...
        try:
            ...
            if nixl_pd_upload_websocket is not None and not is_health_req and self.pd_mode.is_NP():
                ...
                logger.info(f"nixl prefill node upload group_req_id {group_request_id} prompt ids len : {len(prompt_ids)}")
                    ...
                    for req in req_status.group_req_objs.shm_req_objs:
                        ...
                        for _ in range(read_token_count):
                            if not req.out_tokens_queue.is_empty():
                                ...
                                finished_token_index = req.stop_str_matched_token_index if req.stop_str_matched else req.finish_token_index
