...
class InferenceContext:
    ...
    def free_a_req_mem(self, free_token_index: List, req: "InferReq", is_group_finished: bool):
        if self.radix_cache is None:
            if is_group_finished:
                ...
                free_token_index.append(self.req_manager.req_to_token_indexs[req.req_idx][req.shm_req.input_len : req.cur_kv_len])
        else:
    ...
    def get_can_alloc_token_num(self):
        ...
        if self.radix_cache is not None:
            radix_cache_unref_token_num = self.radix_cache.get_tree_total_tokens_num() - self.radix_cache.get_refed_tokens_num()
...
class InferSamplingParams:
    ...
    def has_constraint_setting(self) -> bool:
        return self.regular_constraint is not None or self.allowed_token_ids is not None or self.guided_grammar is not None or self.guided_json is not None
...
class InferReq:
    ...
    def update_finish_status(self, eos_ids, output_len: int):
        ...
        elif output_len > 0 and self.shm_req.shm_prompt_ids.arr[self.shm_req.input_len + output_len - 1] in eos_ids and self.sampling_param.shm_param.ignore_eos is False:
            self.finish_status.set_status(FinishStatus.FINISHED_STOP)
