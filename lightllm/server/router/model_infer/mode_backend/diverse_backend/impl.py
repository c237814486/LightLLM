...
class DiversehBackend(ChunkedPrefillBackend):
    ...
    def beam_prefill(self, event_pack: OverlapEventPack, prefill_reqs: List[InferReq]):
        # 第一阶段
        group_reqs = [g_infer_context.requests_mapping[req.req_id] for req in prefill_reqs if convert_sub_id_to_group_id(req.req_id) == req.req_id]
        groups = [g_infer_context.group_mapping[req.req_id] for req in prefill_reqs if convert_sub_id_to_group_id(req.req_id) == req.req_id]
        ...
        with torch.cuda.stream(g_infer_context.get_overlap_stream()):
            ...
            batch_idx = g_pin_mem_manager.gen_from_list(key="batch_idx_", data=batch_idx, dtype=torch.int64).cuda(non_blocking=True)
            b_req_idx = g_pin_mem_manager.gen_from_list(key="b_req_idx_", data=b_req_idx, dtype=torch.int32).cuda(non_blocking=True)
            b_has_out = g_pin_mem_manager.gen_from_list(key="b_has_out_", data=b_has_out, dtype=torch.bool).cuda(non_blocking=True)
