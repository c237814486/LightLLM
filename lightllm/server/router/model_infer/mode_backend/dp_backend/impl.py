...
class DPChunkedPrefillBackend(ModeBackend):
    ...
    def infer_loop(self):
        ...
        try:
            while True:
                ...
                dp_prefill_req_nums, dp_decode_req_nums = self._dp_all_gather_prefill_and_decode_req_num(prefill_reqs=prefill_reqs, decode_reqs=decode_reqs)
    ...
    def prefill_normal(
        ...
        model_input, run_reqs, padded_req_num = padded_prepare_prefill_inputs(prefill_reqs, is_multimodal=self.is_multimodal)
        ...
        with torch.cuda.stream(g_infer_context.get_overlap_stream()):
            ...
            if run_reqs_num > 0:
                ...
                b_has_out = g_pin_mem_manager.gen_from_list(key="b_has_out", data=b_has_out_cpu, dtype=torch.bool).cuda(non_blocking=True)
    ...
    def prefill_overlap(self, event_pack: OverlapEventPack, prefill_reqs: List[InferReq]):
        ...
        with torch.cuda.stream(g_infer_context.get_overlap_stream()):
            ...
            b_has_out_cpu = micro_input0.b_prefill_has_output_cpu[0:req_num0] + micro_input1.b_prefill_has_output_cpu[0:req_num1]
    ...
    def prefill_overlap_mtp(self, event_pack: OverlapEventPack, prefill_reqs: List[InferReq]):
        ...
        with torch.cuda.stream(g_infer_context.get_overlap_stream()):
            ...
            if req_num1 > 0:
                draft_next_token_ids_gpu1[0:req_num1].copy_(next_token_ids[req_num0 : (req_num0 + req_num1)], non_blocking=True)
            ...
            for draft_model_idx in range(self.mtp_step):
                ...
                draft_micro_output0, draft_micro_output1 = self.draft_models[draft_model_idx].microbatch_overlap_prefill(draft_micro_input0, draft_micro_input1)
    ...
    def decode_overlap_mtp(self, event_pack: OverlapEventPack, decode_reqs: List[InferReq]):
        ...
        with torch.cuda.stream(g_infer_context.get_overlap_stream()):
            ...
            for draft_model_idx in range(self.mtp_step):
                ...
                draft_micro_input0.deepseekv3_mtp_draft_input_hiddens = draft_micro_output0.deepseekv3_mtp_main_output_hiddens
                ...
                draft_micro_output0, draft_micro_output1 = self.draft_models[draft_model_idx].microbatch_overlap_decode(draft_micro_input0, draft_micro_input1)
