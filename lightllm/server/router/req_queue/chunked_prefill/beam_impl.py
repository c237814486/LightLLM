...
class ChunkedBeamContinuesBatchQueue(BaseQueue):
    ...
    def _init_cache_list(self, current_batch: Batch, is_busy):
        if current_batch is not None:
            self.cache_len_list = [(req, req.get_tuple_tokens(is_busy, self.router_max_new_token_len)) for req in current_batch.reqs if req.sample_params.suggested_dp_index == self.dp_index]
        else:
            self.cache_len_list = []
        return
    ...
    def _can_add_new_group_reqs(
        ...
        for req in cur_handle_group_reqs:
            self.cache_len_list.append((req, req.get_tuple_tokens(is_busy, self.router_max_new_token_len)))  # hard to analysis
    ...
    def generate_new_batch(self, current_batch: Batch):
        ...
        for req in self.waiting_req_list:
            ...
            ok_insert, new_batch_first_router_need_tokens = self._can_add_new_group_reqs(cur_group_reqs, is_busy, new_batch_first_router_need_tokens)
            if ok_insert:
