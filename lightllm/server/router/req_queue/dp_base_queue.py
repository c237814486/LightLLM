...
class DpQueue:
    def __init__(self, args, router, base_queue_class, dp_size_in_node) -> None:
        ...
        self.inner_queues: List[BaseQueue] = [base_queue_class(args, router, dp_index, dp_size_in_node) for dp_index in range(self.dp_size_in_node)]
    ...
    def update_token_load(self, current_batch: Batch, force_update=False):
        if self.router.shared_token_load.need_update_dynamic_max_load() or force_update:
            for dp_index in range(self.dp_size_in_node):
                estimated_peak_token_count, dynamic_max_load = self.inner_queues[dp_index].calcu_batch_token_load(current_batch)
    ...
    def _get_suggest_dp_index(self):
        ...
        select_dp_indexes = [i for i, queue in enumerate(self.inner_queues) if len(queue.waiting_req_list) == min_length]
