...
def check_recommended_shm_size(args):
    try:
        ...
        if not is_shm_sufficient:
            ...
            logger.info(
                f"SHM check: Available={shm_size:.2f} GB," f"Recommended={recommended_shm_size:.2f} GB." f"Sufficient: {is_shm_sufficient}",
            )
...
def _get_recommended_shm_size_gb(args, max_image_resolution=(1288, 728), dtype_size=2):
    ...
    if not args.enable_multimodal:
        ...
        if hidden_size is None:
            logger.warning("Model config not contain 'hidden_size', " "using 4096 by default to calculate recommended shm size.")
            image_token_size_bytes = max_image_tokens * 4096 * dtype_size
        else:
            image_token_size_bytes = max_image_tokens * hidden_size * dtype_size

        total_recommended_shm_size_gb = args.cache_capacity * (image_size_bytes + image_token_size_bytes) + req_shm_size_bytes + input_token_logprob_size_bytes
