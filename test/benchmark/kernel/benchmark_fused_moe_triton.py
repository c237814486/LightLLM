...
def benchmark(batch_size, provider, model_config, use_fp8=False, use_cuda_graph=False):
    ...
    # Warmup
    api_func = fused_moe_vllm_api if provider == "vllm_fused_moe_triton" else fused_moe_lightllm_api if provider == "lightllm_fused_moe_triton" else fused_moe_sglang_api
    for _ in range(10):
        api_func(
            x,
            w1,
