...
def parse_args():
    ...
    parser.add_argument(
        "--model_dir", type=str, required=True, help="Directory of the model."
    )
    parser.add_argument(
        "--compare_commit_id",
        type=str,
        default=None,
        help="The commit id of the baseline.",
    )
...
def send_prompts(prompts, output_file):
    for prompt in prompts:
        ...
        try:
            r = requests.post(
                "http://localhost:8080/generate", json=request_data, timeout=10
            )
            response_json = json.loads(r.text)
            generated_text = (
                response_json["generated_text"][0]
                if "generated_text" in response_json
                else "No generated_text."
            )
