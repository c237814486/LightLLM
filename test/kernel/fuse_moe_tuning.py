...
def quantize_moe(weight):
    ...
    for i in range(num_experts):
        qweight, weight_scale = ops.scaled_fp8_quant(weight[i].contiguous().cuda(), scale=None, use_per_token_if_dynamic=False)
        qweights[i] = qweight
...
def test_kernel(
    ...
    **config,
    ...
    if num_fused_shared_experts > 0:
        ...
        pad_topk_ids = (
            torch.arange(start=expert_num - num_fused_shared_experts, end=expert_num, step=1, dtype=topk_ids.dtype, device="cuda").view(1, num_fused_shared_experts).repeat(topk_ids.shape[0], 1)
        )
