...
@pytest.mark.parametrize(
    ...
    [(token_num, hidden_dim, sp_world_size) for token_num in range(3, 6) for hidden_dim in [257, 2048] for sp_world_size in range(2, 5)],
)
def test_sp_pad_copy(token_num, hidden_dim, sp_world_size):
    ...
    out_tensors = [sp_pad_copy(in_tensor=in_tensor, sp_rank_id=rank_id, sp_world_size=sp_world_size) for rank_id in range(sp_world_size)]
