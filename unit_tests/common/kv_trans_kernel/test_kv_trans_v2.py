...
def test_kv_trans_v2_for_p_node(token_num):
    ...
    for _ in range(dp_size_in_node):
        mems.append(
            torch.randn(
                (kv_buffer_token_num, head_num, head_dim),
                dtype=torch.float16,
                device="cuda",
            )
        )
    input_mems = torch.tensor(
        [e.data_ptr() for e in mems], dtype=torch.uint64, device="cuda"
    )
    ...
    true_output = torch.zeros(
        (token_num, head_num, head_dim), dtype=torch.float16, device="cuda"
    )
    test_output = torch.zeros(
        (token_num, head_num, head_dim), dtype=torch.float16, device="cuda"
    )
    output_idx = torch.arange(0, token_num, 1, dtype=torch.int32, device="cuda")

    kv_trans_v2_for_p_node(
        input_mems, input_idx, input_dp_idx, test_output, output_idx, dp_size_in_node
    )
...
def test_kv_trans_v2_for_d_node(token_num):
    ...
    kv_trans_v2_for_d_node(
        output_mems, output_idx, output_dp_idx, test_input, input_idx, dp_size_in_node
    )
    ...
    for dest_token_index, token_index, dest_token_index, dp_index in zip(
        ...
        for mem_index in range(
            dp_index * card_num // dp_size_in_node,
            (dp_index + 1) * card_num // dp_size_in_node,
        ):
            torch.equal(
                mems[mem_index][dest_token_index, :, :], test_input[token_index, :, :]
            )
