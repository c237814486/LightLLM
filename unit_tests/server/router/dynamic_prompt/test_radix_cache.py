...
def test_case2():
    ...
    tree_node, size, values = tree.match_prefix(
        torch.tensor([0, 1, 2, 3, 4], dtype=torch.int64, device="cpu"),
        update_refs=False,
    )
    assert tree_node.node_prefix_total_len == 5 and size == 5 and len(values) == 5
    tree_node, size, values = tree.match_prefix(
        torch.tensor([0, 1, 2, 3, 4, 9], dtype=torch.int64, device="cpu"),
        update_refs=False,
    )
    assert tree_node.node_prefix_total_len == 5 and size == 5 and len(values) == 5
    tree_node, size, values = tree.match_prefix(
        torch.tensor([0, 1, 2, 3, 4, 7, 8], dtype=torch.int64, device="cpu"),
        update_refs=False,
    )
    assert tree_node.node_prefix_total_len == 7 and size == 7 and len(values) == 7
    tree_node, size, values = tree.match_prefix(
        torch.tensor([0, 1, 2, 3, 4, 7, 9], dtype=torch.int64, device="cpu"),
        update_refs=False,
    )
...
def test_case3():
    ...
    tree_node, size, values = tree.match_prefix(torch.tensor([0, 1, 2, 3, 4], dtype=torch.int64, device="cpu"), update_refs=True)
    assert tree_node.node_prefix_total_len == 5 and size == 5 and len(values) == 5
